        ...
        Column(
            ...
            Card(Modifier.fillMaxWidth()) {
                Column(Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(4.dp)) {
                    ...
                    FeatureBullet("On-device GGUF models (llama.cpp): Vulkan GPU offload with CPU fallback, resumable downloads, import/export, offline chat")
                    FeatureBullet("In-app update checker: silent start-up check, notification, one-tap per-ABI APK download")
                    FeatureBullet("Chat backup: export/import every conversation as a JSON file via the system picker")
